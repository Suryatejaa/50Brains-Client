...
export const deleteNotification = async (
  id: string
): Promise<{ success: boolean }> => apiClient.delete(`/api/notification/${id}`);
...
export const getUserAnalytics = async (
  userId: string
): Promise<{ success: boolean; data: NotificationAnalytics }> =>
...
export const previewEmailTemplate = async (
  templateName: string
): Promise<{ success: boolean; data: { html: string; text: string } }> =>
 ...
 */
export const testAPIGatewayConnection = async (): Promise<{
  success: boolean;
  message: string;
}> => {
  try {
...
export const subscribeToClanChannels = async (
  ...
  try {
    ...
    return {
      success: true,
      message: `Subscribed to ${channels.length} clan channels`,
    };
  } catch (error: any) {
...
export const unsubscribeFromClanChannels = async (
  ...
  try {
    ...
    const response = await apiClient.post(
      '/api/notification/clan/unsubscribe',
      {
        userId,
        channels,
      }
    ...
    return {
      success: true,
      message: `Unsubscribed from ${channels.length} clan channels`,
    };
  } catch (error: any) {
...
export const getClanNotificationChannels = async (
  ...
  try {
    const response = await apiClient.get(
      `/api/notification/clan/channels/${userId}`
    );
    if (response.success) {
      return {
        success: true,
        data: { channels: (response.data as any)?.channels || [] },
      };
    }
...
export const testClanWebSocketConnection = async (
  ...
  return new Promise((resolve) => {
    ...
    ws.onopen = () => {
      ...
      resolve({
        success: true,
        message: 'Clan WebSocket connection successful',
      });
    };
    ...
    ws.onerror = (error) => {
      ...
      resolve({
        success: false,
        message: `Clan WebSocket connection error: ${error}`,
      });
    };
  });
};
