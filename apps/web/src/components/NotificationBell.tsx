...
export default function NotificationBell() {
  ...
  const displayUnreadCount =
    optimisticUnreadCount !== null ? optimisticUnreadCount : unreadCount;
  ...
  useEffect(() => {
    ...
    if (!isConnected && connectionStatus === 'disconnected') {
      ...
      const reconnectTimer = setTimeout(() => {
        if (!isConnected) {
          console.log(
            '🔄 Triggering force refresh to check for missed notifications'
          );
          forceRefresh();
        }
      }, 2000);
  ...
  const handleBellClick = async () => {
    ...
    console.log(
      '🔔 Bell clicked - forcing refresh to catch missed notifications'
    );
    await forceRefresh();
  ...
  return (
    <div className="relative" ref={dropdownRef} style={{ zIndex: 99999 }}>
        ...
        {displayUnreadCount > 0 && (
          <>
            ...
            {displayUnreadCount > 0 && showDot ? (
              <span className="absolute right-0 top-0 flex h-3 w-3 items-center justify-center rounded-full bg-red-500 transition-all duration-300 ease-in-out sm:right-1 sm:top-1">
                <div className="h-1.5 w-1.5 rounded-full bg-white"></div>
              </span>
            ) : (
              /* Show count or "9+" for counts > 9 initially */
              <span className="absolute right-0 top-0 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs font-medium text-white transition-all duration-300 ease-in-out sm:right-1 sm:top-1">
      ...
      {open && (
        <div
          className="absolute -right-8 mt-2 w-80 rounded-lg border border-gray-200 bg-white shadow-xl"
          style={{ zIndex: 999999 }}
        >
          {/* Header */}
            ...
            {loading ? (
              ...
              notifications.map((notification) => {
                ...
                return (
                  <div
                    key={notification.id}
                    className={`cursor-pointer border-b border-l-4 border-gray-100 px-4 py-3 transition-colors hover:bg-gray-50 ${getPriorityColor(notification.priority)} ${!notification.read ? 'bg-blue-50' : ''}`}
                    onClick={async (e) => {
                      e.stopPropagation();
                  ...
                  >
                    <div className="flex items-start space-x-3">
                      <div className="min-w-0 flex-1">
                        <div
                          className={`truncate text-sm font-medium ${!notification.read ? 'text-gray-900' : 'text-gray-700'}`}
                        >
                          {notification.title}
                        </div>
