...
export function useClans(initialFilters: ClanFilters = {}) {
  ...
  const [pagination, setPagination] = useState<ClansResponse['meta'] | null>(
    null
  );
  ...
  if (!isFeatureEnabled('CLANS_ENABLED')) {
    return {
      ...
      getClanFeed: async () => {},
      getMyClans: async () => {},
      getPublicClans: async () => {},
      setError: () => {},
    };
  }

  const fetchClans = useCallback(
    async (newFilters?: ClanFilters) => {
      ...
      try {
        ...
        if (response.success) {
          ...
          setPagination(
            responseData?.pagination || (response as any).meta || null
          );
        } else {
...
export function useClan(clanId: string) {
  ...
  if (!isFeatureEnabled('CLANS_ENABLED')) {
    return {
      ...
      error: null,
      refetch: async () => {},
      updateClan: async () => false,
      deleteClan: async () => false,
      setError: () => {},
    };
  }
...
export function useClanJoinRequests(clanId: string) {
  ...
  const fetchRequests = useCallback(async () => {
    if (!clanId || !isAuthenticated) {
      console.log(
        'Skipping join requests fetch - missing clanId or not authenticated'
      );
      return;
    }
    ...
    try {
      ...
      if (response.success) {
        ...
        if (errorMessage.includes('not authorized')) {
          console.warn(
            'User not authorized to view join requests for this clan'
          );
  ...
  const reject = useCallback(
    async (requestId: string, reason?: string) => {
      ...
      try {
        setLoading(true);
        setError(null);
        const res = await clanApiClient.rejectJoinRequest(
          clanId,
          requestId,
          reason ? { reason } : {}
        );
