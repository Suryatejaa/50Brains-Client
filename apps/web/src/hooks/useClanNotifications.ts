...
export function useClanNotifications() {
  ...
  if (
    ...
    !isFeatureEnabled('CLAN_NOTIFICATIONS')
  ) {
    return {
      clanChannels: [],
      clanNotifications: [],
      subscribeToUserClans: async () => {},
      unsubscribeFromClans: async () => {},
      fetchClanNotificationChannels: async () => {},
      getClanNotificationsByClan: () => [],
      getClanNotificationsByGig: () => [],
      getClanNotificationsByType: () => [],
      getUserClanIds: () => [],
      getUnreadClanNotificationCount: () => 0,
      markClanNotificationProcessed: () => {},
      totalClanNotifications: 0,
      hasClanNotifications: false,
    };
  }
  const {
    clanChannels,
  ...
  const unsubscribeFromClans = useCallback(
    async (clanIds: string[]) => {
      if (clanIds.length > 0) {
  ...
  const getClanNotificationsByClan = useCallback(
    (clanId: string) => {
      return clanNotifications.filter(
        (notification) => notification.data.clanId === clanId
      );
    },
    [clanNotifications]
  );
  ...
  const getClanNotificationsByGig = useCallback(
    (gigId: string) => {
      return clanNotifications.filter(
        (notification) => notification.data.gigId === gigId
      );
    },
    [clanNotifications]
  );
  ...
  const getClanNotificationsByType = useCallback(
    (type: string) => {
      return clanNotifications.filter(
  ...
  const markClanNotificationProcessed = useCallback(
