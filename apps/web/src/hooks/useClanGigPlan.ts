...
export const useClanGigPlan = () => {
    ...
    const createClanTask = useCallback(async (
        clanId: string,
        gigId: string,
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'Failed to create clan task');
            return null;
        } finally {
            setLoading(false);
        }
    ...
    const updateClanTask = useCallback(async (
        clanId: string,
        gigId: string,
        taskId: string,
        ...
        try {
            const result = await clanApiClient.updateClanTask(clanId, gigId, taskId, data);
            return result.data as ClanWorkPackage;
        } catch (err) {
            setError(err instanceof Error ? err.message : 'Failed to update clan task');
            return null;
        } finally {
            setLoading(false);
        }
    ...
    const getClanTasks = useCallback(async (
        clanId: string,
        gigId: string
    ): Promise<ClanWorkPackage[] | null> => {
        setLoading(true);
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'Failed to fetch clan tasks');
            return null;
        } finally {
            setLoading(false);
        }
    ...
    const getClanGigAssignments = useCallback(async (
        clanId: string
    ) => {
        setLoading(true);
        setError(null);
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'Failed to fetch clan gig assignments');
            return null;
        } finally {
            setLoading(false);
        }
    ...
    const getClanMemberAgreements = useCallback(async (
        clanId: string,
        gigId: string
    ): Promise<MemberAgreement[] | null> => {
        setLoading(true);
